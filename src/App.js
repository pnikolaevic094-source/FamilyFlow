    ...
    <div className="page" style={{ padding: '60px 20px', textAlign: 'center' }}>
      ...
      {inviteInfo && (
        ...
        >
          ...
          <p>
            Семья: <strong>{decodeURIComponent(inviteInfo.familyName)}</strong>
          </p>
          <p>
            Роль:{' '}
            <strong>
              {inviteInfo.role === 'parent'
                ? '👨 Родитель'
                : inviteInfo.role === 'grandparent'
                ? '👴 Бабушка/Дедушка'
                : '👶 Ребёнок'}
            </strong>
          </p>
          <p style={{ fontSize: '14px', opacity: 0.9 }}>
            Зарегистрируйтесь или войдите, чтобы присоединиться
          </p>
        </div>
      )}
      ...
      >
        ...
        <div className="card" style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>
            {
              JSON.parse(localStorage.getItem('familyflow_tasks') || '[]').filter(
                (t) => t.completed
              ).length
            }
          </div>
        ...
        <div className="card" style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>
            {
              JSON.parse(localStorage.getItem('familyflow_tasks') || '[]').filter(
                (t) => !t.completed && (!t.startDate || new Date(t.startDate) <= new Date())
              ).length
            }
          </div>
...
function CalendarPage() {
  const [events, setEvents] = useState(
    JSON.parse(localStorage.getItem('familyflow_events') || '[]')
  );
  ...
  const getEventsForDate = (date) => {
    ...
    return [
      ...
      ...tasks
        .filter((t) => t.createdAt && new Date(t.createdAt).toDateString() === dateStr)
        .map((t) => ({ ...t, type: 'task' })),
    ];
  };
                  ...
                  <>
                    ...
                    {dayEvents.slice(0, 2).map((event, i) => (
                      ...
                      >
                        ...
                        {event.location && (
                          <div style={{ fontSize: '9px' }}>📍 {event.location}</div>
                        )}
                      </div>
                    ))}
                    {dayEvents.length > 2 && (
...
function ChatPage() {
  const [messages, setMessages] = useState(
    JSON.parse(localStorage.getItem('familyflow_messages') || '[]')
  );
...
const ACHIEVEMENTS = [
  ...
  },
  {
    ...
    name: 'Общительный',
    icon: '💬',
    desc: 'Отправьте 5 сообщений',
    check: (data) => data.messages >= 5,
  },
  {
    id: 'perfectionist',
    name: 'Перфекционист',
    icon: '✨',
    desc: '100% выполнение',
    check: (data) => data.totalTasks > 0 && data.progress === 100,
  },
  {
    id: 'priority_master',
    name: 'Мастер приоритетов',
    icon: '🎯',
    desc: 'Создайте задачи с разными приоритетами',
    check: (data) => data.priorityTypes >= 3,
  },
];
...
function FamilyPage() {
  const [familyName, setFamilyName] = useState(
    localStorage.getItem('familyflow_family_name') || ''
  );
  const [members, setMembers] = useState(
    JSON.parse(localStorage.getItem('familyflow_members') || '[]')
  );
  ...
  const currentUser = LocalAuth.currentUser;
  const userInFamily = members.find(
  ...
  const addMember = (e) => {
    ...
    const newMember = {
      ...
      role: selectedRole,
      joinedAt: new Date().toISOString(),
      inviteLink: `${window.location.origin}?invite=${inviteId}&family=${encodeURIComponent(
        familyName
      )}&role=${selectedRole}`,
      status: 'pending',
    };
          ...
          <div className="card" style={{ marginBottom: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <p>
                  Участников: {members.length} • Ваша роль:{' '}
                  {userInFamily?.role === 'parent'
                    ? '👨 Родитель'
                    : userInFamily?.role === 'grandparent'
                    ? '👴 Бабушка/Дедушка'
                    : '👶 Ребёнок'}
                </p>
              </div>
                ...
                >
                  <div
                    style={{
                      ...
                      justifyContent: 'space-between',
                      alignItems: 'center',
                      marginBottom: member.inviteLink ? '10px' : '0',
                    }}
                  >
                    <div>
                      ...
                      <div style={{ fontSize: '14px', color: 'var(--text-secondary)' }}>
                        {member.role === 'parent'
                          ...
                          : '👶 Ребёнок'}
                        {member.email &&
                          !member.email.includes('@familyflow.local') &&
                          ` • ${member.email}`}
                        ...
                        {member.status === 'active' &&
                          member.email &&
                          member.email.includes('@familyflow.local') && (
                            <span style={{ color: 'var(--success)', marginLeft: '10px' }}>
                              👥 Гость
                            </span>
                          )}
                      </div>
                        ...
                        >
                          <div
                            style={{
                              width: '100px',
                              height: '100px',
                              background: `url('https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(
                                member.inviteLink
                              )}')`,
          ...
          <div className="search-box" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginTop: '15px', maxHeight: '300px', overflowY: 'auto' }}>
              ...
              {searchQuery && searchResults().length === 0 && (
                <div
                  style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: '20px' }}
                >
                  Ничего не найдено
                </div>
              )}
